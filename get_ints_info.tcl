...
set ints [get_tiles * -filter {TYPE==INT_L || TYPE==INT_R}]
# set ints_r [get_tiles * -filter {TYPE==INT_R}]

set fp [open /media/brookz/Storage/Research/FPGA-RE/int_conn_map5.json w]
...
puts $fp "{"
set i 0
set len1 [llength $ints]

foreach int $ints {
    incr i
    puts $fp [format "\t\"%s\" : \{" $int]
    set j 0
    set wires [get_wires -of_objects $int -filter {IS_CONNECTED}] 
    set len [llength $wires]

    foreach wire $wires {
        incr j
        set wire [format "%s" $wire]
        set wire [string range $wire [string first "/" $wire]+1 end]
        ...
        if {[llength $conns] == 1 && [lindex $conns 0] == $wireStr && [string first "GFAN" $wireStr] == -1 && [string first "BEG" $wireStr] == -1} {
            set conns [get_nodes -downhill -of_objects [get_nodes $wireStr]]
        } elseif {[llength $conns] > 1} {
            set conns [lrange $conns 1 end]
        } else {
            set conns [get_nodes -downhill -of_objects [get_wires $wireStr]]
        }
        if {[llength $conns] <= 1} {
            set fmt "\"%s\""
        } else {
